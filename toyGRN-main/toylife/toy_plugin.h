...
struct Dim {
    ...
    Prot p2;
    bool operator<(
        const Dim& rhs) const {  // in order to compare and use this in maps
        if (id < rhs.id)
            return 1;
        else if (id == rhs.id)
            if (p1 < rhs.p1)
...
struct OWM {
    ...
    Met met;
    bool operator<(
        const OWM& rhs) const {  // in order to compare and use this in maps
        if (prot < rhs.prot)
            return 1;
        else if (prot == rhs.prot)
            if (dim < rhs.dim)
                return 1;
            else if (dim.id == rhs.dim.id && dim.p1 == rhs.dim.p1 &&
                     dim.p2 == rhs.dim.p2)
                if (met < rhs.met)
                    return 1;
        return 0;
...
struct KeyHash {
    std::size_t operator()(const pairDM& k) const {
        ...
        return seed;  // std::hash<std::string>()(k.first)^(std::hash<int>()(k.second
                      // << 1));
    }
};
...
typedef std::unordered_map<pairPM, double, KeyHash, KeyEqual>
    mapa_pmet;  // for proteinas_metabolitos
typedef std::map<std::pair<int, std::string>, std::pair<double, int> >
    mapa_pbreak;
...
class ToyPlugin {
   public:
    ...
    void dimerization(std::pair<mapa_prot, mapa_dim>& objects) const; //maybe move to agent??

    // FUNCTIONS to delete
    // std::vector<int> regulatory_phenotype(const std::string& genotype);
    // std::vector<int> metabolic_phenotype(const std::string& genotype,
    //                                      const std::vector<std::string>& env);
    // double metabolic_robustness(const std::string& genotype,
    //                             const std::vector<int>& phen,
    //                             const std::vector<std::string>& env);
    // double regulatory_robustness(const std::string& genotype,
    //                              const std::vector<int>& logic_vector);
    // std::map<std::vector<int>, int> metabolic_evolvability(
   ...
   private:
    ...
    void init_prot(std::vector<int>& perimeters, std::vector<double>& energies,
                   std::vector<std::vector<double> >& prot_prom_energies,
                   std::vector<std::vector<bool> >& prot_pol);
    void init_dim(std::map<std::pair<int, int>, int>& dim_perim,
                  std::vector<double>& dim_energy,
                  std::vector<std::vector<double> >& dim_prom_energies,
                  std::vector<std::vector<bool> >& dim_pol);
    void init_polymerase(std::vector<double>& polymerase);
    void simplified_mets(std::vector<std::string>& mets,
                         std::map<std::string, int>& not_broken,
                         std::vector<std::string>& env);
    void neighbors(std::vector<std::vector<int> >& neighbors_prom,
                   std::vector<std::vector<int> >& neighbors_coding);
...
// to delete
    ...
    // int promoter_expression(const std::vector<std::pair<int, int> >& genotype,
    ...
    //std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim> > >
    // boolean_network(const std::vector<std::pair<int, int> >& genotype);
    // std::vector<int> complete_boolean_function(
    //     const std::vector<std::pair<int, int> >& genotype);
    // void reacting(std::pair<mapa_prot, mapa_dim>& objects, mapa_met& met,
    //               mapa_owm& objects_with_mets, double& fit);
    // int metabolism(const std::vector<std::pair<int, int> >& genotype,
    //                std::pair<mapa_prot, mapa_dim> objects, mapa_met& met);
    // std::vector<int> vec_phenotype(
    //     const std::vector<std::pair<int, int> >& genotype,
    //     const std::vector<std::string>& env);
};
