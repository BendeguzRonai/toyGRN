...
inline std::string decto4(int decimal, const int size) {
    std::string base4(size, '0');
    for (int i = size - 1; i >= 0; --i) {
        int power = std::pow(4, i);
        int digit = decimal / power;
        base4[size - i - 1] = '0' + digit;  // Convert digit to char
        decimal -= digit * power;
    }
    return base4;
}

inline int base4todec(const std::string& base4) {
    int decimal = 0;
    int size = base4.size();
    for (int i = 0; i < size; ++i) {
        int digit = base4[i] - '0';  // Convert char to int
        decimal += digit * std::pow(4, size - i - 1);
    }
    return decimal;
}
