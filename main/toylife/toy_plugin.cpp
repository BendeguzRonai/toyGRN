...
// STRUCTURE FUNCTIONS
Dim::Dim() : id(-1), p1(0), p2(0) {}
Dim::Dim(const int Id, const int P1, const int P2) : id(Id), p1(P1), p2(P2) {}
bool Dim::empty() const { return (id == -1); }
OWM::OWM() : prot(-1), dim(), met() {}
OWM::OWM(const Prot p1, const Dim& d1, const Met& m1)
    : prot(p1), dim(d1), met(m1) {}
bool OWM::empty() const { return (prot == -1 && dim.empty() && met.empty()); }
Dmet::Dmet() : eg(0.0), seq(), pos(0) {}
...
void ToyPlugin::init_prot(
    std::vector<int>& perimeters, std::vector<double>& energies,
    std::vector<std::vector<double> >& prot_prom_energies,
    std::vector<std::vector<bool> >&
        prot_pol) {  // reads to table the perimeter and energies associated to
                     // each protein
    // NUMBER OF PROTEINS
    ...
    while (in >> id >> en) {
        energies[id] = en;
        for (int j = 0; j < 8; j++)  // each protein has 8 sides (4 for each
                                     // isomer), but we don't need them now
            in >> dirt;
    }
    in.close();
...
void ToyPlugin::init_dim(
    std::map<std::pair<int, int>, int>& dim_perim,
    std::vector<double>& dim_energy,
...
////////////////////////////////////////////////////////////////////////
// DIM_MET
void ToyPlugin::init_dim_met(
    mapa_dmet&
        dim_met) {  // key: Met; value: map -> key (Id of the dimer), value:
                    // Dmet, structure with the following members: (eg) binding
                    // energy, (seq) sequence left for proteins to bind to,
                    // (pos) how many sugars are to the left of the center
    for (int i = 4; i < 9; ++i) {
        ...
        std::stringstream ss;
        ss << i;
        for (std::map<int, std::vector<int> >::const_iterator it = mapa.begin();
            ...
            while (file_dimeros_mets >> index) {
                ...
                dim_met[std::make_pair(met_id, index)] =
                    Dmet(energy, sequence, position);
            }
...
void ToyPlugin::init_prot_met(mapa_pmet& prot_met) {
    for (int i = 4; i < 9; ++i) {
        ...
        ss << i;
        for (std::map<int, std::vector<int> >::const_iterator it = mapa.begin();
             it != mapa.end(); ++it) {  // for all the sequences
...
// PROMOTER_EXPRESSION to delete
// int ToyPlugin::promoter_expression(
//     const std::vector<std::pair<int, int> >& genotype,
...
//             binding_energies;
//         for (mapa_prot::const_iterator it_prot = objects.first.begin();
//              it_prot != objects.first.end();
//              ++it_prot)  // for every protein in OBJECTS.FIRST
//             if (d_less(prot_prom_energies[prom][it_prot->first],
//                        0.0) &&  // if the protein can bind to PROM AND
//                 d_less(prot_prom_energies[prom][it_prot->first] +
//                            prot_energies[it_prot->first] - polymerase[prom],
//                        0.0))  // it can bind it more strongly than POLY
//                 binding_energies.first[it_prot->first] =
//                     prot_prom_energies[prom][it_prot->first] +
//                     prot_energies[it_prot->first] -
//                     polymerase[prom];  // we substract the binding energy of the
//                                        // polymerase; if it is greater than
//                                        // zero, binding energy is zero.
//         for (mapa_dim::const_iterator it_dim = objects.second.begin();
//              it_dim != objects.second.end();
//              ++it_dim)  // for every dimer in OBJECTS.SECOND
//             if (d_less(dim_prom_energies[prom][it_dim->first.id],
//                        0.0) &&  // if the dimer can bind to PROM AND
//                 d_less(dim_prom_energies[prom][it_dim->first.id] +
//                            dim_bond_energy[it_dim->first.id] +
//                            prot_energies[it_dim->first.p1] +
//                            prot_energies[it_dim->first.p2] - polymerase[prom],
//                        0.0))  // it can bind it more strongly than POLY
//                 binding_energies.second[it_dim->first] =
//                     dim_prom_energies[prom][it_dim->first.id] +
//                     dim_bond_energy[it_dim->first.id] +
//                     prot_energies[it_dim->first.p1] +
//                     prot_energies[it_dim->first.p2] -
//                     polymerase[prom];  // we substract the binding energy of the
//                                        // polymerase; if it is greater than
//                                        // zero, binding energy is zero
...
//                 if (!i_min)
//                     promoter_active[p] =
//                         prot_pol[prom][prot_min->first];  // return the minimum
//                 else
//                     promoter_active[p] =
//                         dim_pol[prom][dim_min->first.id];  // return the minimum
//                 break;                                     // we exit the loop
//             }
//         }  // while TRUE
//     }      // for promoters
...
//     // EXPRESSION
//     objects.first.clear();  // now we know which promoters are expressed, all
//                             // objects disappear
//     objects.second.clear();
//     for (int i = 0; i < promoter_active.size(); ++i)
//         if (promoter_active[i])
//             objects
//                 .first[prot_gen[genotype[i].second]]++;  // we add the proteins
//                                                          // that are expressed

//     // OUTPUT
//     int translated =
//         0;  // as every gene has a place in the genotype, we can express every
//             // state as an integer: (1,1,0) = 2^2+2^1 = 6
//     for (int i = 0; i < promoter_active.size(); ++i)
//         translated +=
//             promoter_active[i] * int_pow(2, promoter_active.size() - i - 1);
//     return translated;
// }
...
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
// FINDING_CYCLES to delete
// bool ToyPlugin::finding_cycles(
//     std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim> > >& iterations,
//     int expression) {
//     // OUTPUT
//     // Given a vector of all visited states and the last state visited,
//     // TERNA_FINDING_CYCLES checks if this state has been visited before. If it
//     // has, then it transforms the ITERATIONS vector and leaves only the states
//     // belonging to the cycle; it returns TRUE. If it hasn't, it returns FALSE.
...
//         if (iterations[i].first ==
//             expression) {  // if, at any point, this state has been visited
//                            // before, we have a cycle
//             iterations.erase(
//                 iterations.begin(),
//                 iterations.begin() + i);  // we erase all the previous states,
...
////////////////////////////////////////////////////////////////////////
// DIMERIZATION
void ToyPlugin::dimerization(std::pair<mapa_prot, mapa_dim>& objects) const {
    // WHICH DIMERS FORM
    std::map<double, mapa_dim>
        binding_energies;  // creates a map of energies, so that dimers with the
                           // same final energy are classified in the same
                           // group. Each energy has a map of dimers, consisting
                           // of the dimer and the number of times it can be
                           // formed
    for (mapa_prot::const_iterator it1 = objects.first.begin();
         it1 != objects.first.end();
         ++it1) {  // we only want to explore the combinations of two proteins
        for (mapa_prot::const_iterator it2 = it1; it2 != objects.first.end();
             ++it2) {  // remember that proteins are sorted (and that they can
                       // form dimers with themselves)
            if (it1 != it2 ||
                (it1 == it2 &&
                 it1->second >
                     1)) {  // Prots will only dimerize with themselves if there
                            // is more than one copy
                std::pair<int, int> perim = std::make_pair(
                    prot_perimeters[it1->first], prot_perimeters[it2->first]);
                std::map<std::pair<int, int>, int>::const_iterator it_dim =
                    dim_perim.find(perim);
                if (it_dim != dim_perim.end()) {  // if this pair forms a dimer,
                                                  // we form the tuple
                    double b_energy = dim_bond_energy[it_dim->second] +
                                      prot_energies[it1->first] +
                                      prot_energies[it2->first];
                    binding_energies[b_energy][Dim(it_dim->second, it1->first,
                                                   it2->first)] +=
    ...
    for (std::map<double, mapa_dim>::iterator it = binding_energies.begin();
         it != binding_energies.end(); ++it) {
        std::map<Prot, int>
            number;  // will hold how many complexes each protein is involved in
                     // (if it's greater than objects.first[i], then no dimers
                     // is formed
        for (mapa_dim::iterator it_dim = it->second.begin();
             it_dim != it->second.end(); ++it_dim) {  // for each possible dimer
            number[it_dim->first.p1] += it_dim->second;
            number[it_dim->first.p2] += it_dim->second;
        }
        // Now, for every possible complex, we check if the proteins involved
        // are involved in too many complexes
        for (mapa_dim::iterator it_dim = it->second.begin();
             it_dim != it->second.end(); ++it_dim)  // for each dimer
            if (objects.first[it_dim->first.p1] <
                    number[it_dim->first.p1] ||  // if either of the two prots
                                                 // take part
                objects.first[it_dim->first.p2] <
                    number[it_dim->first.p2])  // in more dimers that they can
    ...
    for (mapa_prot::iterator it1 = objects.first.begin();
         it1 != objects.first.end(); ++it1)
        if (it1->second == 0)
            objects.first.erase(
                it1);  // eliminate the proteins that have 0 copies
}
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
// BOOLEAN NETWORK to delete
// std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim> > >
// ToyPlugin::boolean_network(const std::vector<std::pair<int, int> >& genotype) {
//     // COMPUTES THE ATTRACTOR OF THE DYNAMICS OF A toyLIFE ORGANISM
//     std::pair<mapa_prot, mapa_dim> objects;
//     std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim> > > cycle(
//         1, std::make_pair(0, objects));                     // first state is 0
//     int nu_state = promoter_expression(genotype, objects);  // compute next
//                                                             // state
...
//         if (finding_cycles(cycle,
//                            new_state))  // if the state has already been visited
//             break;              // this is the only way out of the while loop
//         dimerization(objects);  // dimerize
//         cycle.push_back(std::make_pair(
//             new_state,
//             objects));  // if the state hasn't been visited, we keep looking
//     }
...
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
// // COMPLETE BOOLEAN FUNCTION to delete
// std::vector<int> ToyPlugin::complete_boolean_function(
//     const std::vector<std::pair<int, int> >& genotype) {
//     // COMPUTES THE ATTRACTOR OF THE DYNAMICS OF A toyLIFE ORGANISM
//     int g = genotype.size();
...
//         dimerization(objects);  // dimerize
//         int nu_state =
//             promoter_expression(genotype, objects);  // compute next state
...
////////////////////////////////////////////////////////////////////////
// REACTING to delete
// void ToyPlugin::reacting(std::pair<mapa_prot, mapa_dim>& objects, mapa_met& met,
//                          mapa_owm& objects_with_mets, double& fit) {
//     // GIVEN A SET OF OBJECTS, IT TELLS US WHAT BINDS WHAT AND WHAT BREAKS WHAT
//     std::map<double, mapa_owm>
//         binding_energies;  // creates a map of energies, so that complexes with
//                            // the same final energy are classified in the same
//                            // group. Each energy has a map of OWM, which has
//                            // three members: prot, dim and met (if there is no
//                            // Prot in the complex, it will be -1; if there is no
//                            // Dim, it will be (-1,0,0), and if there is no Met
//                            // it will be the empty string, "") and [VALUE] an
//                            // int counting how many possible molecules can be
//                            // formed of this type
...
//                                                   // we form the tuple
//                     double b_energy =
//                         dim_bond_energy[it_dim->second] +
//                         prot_energies[it1->first] +
//                         prot_energies[it2->first];  // energy of the complex
...
//     for (mapa_prot::const_iterator it_prot = objects.first.begin();
//          it_prot != objects.first.end(); ++it_prot)
//         for (mapa_met::const_iterator it_met = met.begin(); it_met != met.end();
//              ++it_met) {
...
//                 if (d_less(e1, 0.0)) {  // if the protein can bind the
//                                         // metabolite
//                     double b_energy =
//                         e1 +
//                         prot_energies[it_prot->first];  // energy of the complex
...
//     for (mapa_dim::const_iterator it_dim = objects.second.begin();
//          it_dim != objects.second.end(); ++it_dim)
//         for (mapa_met::const_iterator it_met = met.begin(); it_met != met.end();
//              ++it_met) {
...
//                     double b_energy =
//                         tupla.eg + dim_bond_energy[it_dim->first.id] +
//                         prot_energies[it_dim->first.p1] +
//                         prot_energies[it_dim->first.p2];  // energy of the
//                                                           // complex
...
//     for (mapa_owm::const_iterator it_dm = objects_with_mets.begin();
//          it_dm != objects_with_mets.end(); ++it_dm) {
...
//                 continue;  // only check this if the D+M can actually be broken!
//             std::pair<int, std::string> par2(it_prot->first, tupla.seq);
//             try {  // look and see if the protein can break this dimer
//                 double b_energy = prot_breaking.at(par2).first +
//                                   prot_energies[it_prot->first];
//                 int c_min = prot_breaking.at(par2).second;
//                 if (c_min <
//                     2)  // the sequences should be ordered in such a way that
//                         // the first half is always the first protein
//                     b_energy +=
//                         prot_energies[it_dm->first.dim.p1];  // sum energy of
//                                                              // first subunit
//                 // if ((c_min<2 && tupla.bind<3) ||//if the prot binds to the
//                 // first subunit when the dimer is not reversed
//                 //     (c_min>1 && tupla.bind>2))//or if it binds to the second
//                 //     subunit when the dimer is reversed
//                 //   b_energy += prot_energies[it_dm->first.dim.p1];//sum energy
//                 //   of first subunit
//                 else
//                     b_energy +=
//                         prot_energies[it_dm->first.dim.p2];  // sum energy of
//                                                              // second subunit
//                 double met_energy =
//                     dim_bond_energy[it_dm->first.dim.id] +
...
//                 number_mets[it_tuple->first.met] += it_tuple->second;
//             }
//         // NOW, FOR EVERY POSSIBLE COMPLEX, WE CHECK IF THE UNITS ARE INVOLVED
//         // IN TOO MANY COMPLEXES AND DISCARD THOSE
//         for (mapa_owm::iterator it_tuple = it->second.begin();
//              it_tuple != it->second.end(); ++it_tuple)  // for each complex
//             if (it_tuple->first.met.empty()) {  // protein-protein complex
//                 if (objects.first[it_tuple->first.dim.p1] <
//                         number_prots[it_tuple->first.dim.p1] ||  // prot1 more
//                                                                  // involved
//                     objects.first[it_tuple->first.dim.p2] <
...
//                     if (objects.first[it_tuple->first.prot] <
//                             number_prots[it_tuple->first.prot] ||  // proteins
//                                                                    // more
//                                                                    // involved
//                         met[it_tuple->first.met] <
...
//             }       // proteins and mets are involved
//             else {  // dimer-met complex
//                 if (objects.second[it_tuple->first.dim] <
...
//              it_tuple != it->second.end(); ++it_tuple)  // for each complex
//             if (it_tuple->first.met.empty()) {  // protein-protein complex
//                 objects.second[it_tuple->first.dim] +=
//                     it_tuple->second;  // we add as many copies as possible
//                 objects.first[it_tuple->first.dim.p1] -=
...
//                 } else {  // protein breaking a d+m complex
//                     objects_with_mets[OWM(-1, it_tuple->first.dim,
//                                           it_tuple->first.met)] -=
//                         it_tuple->second;
//                     objects.first[it_tuple->first.prot] -= it_tuple->second;
...
//     for (mapa_prot::iterator it_prot = objects.first.begin();
//          it_prot != objects.first.end(); ++it_prot)
//         if (it_prot->second == 0)
//             objects.first.erase(
//                 it_prot);  // eliminate the proteins that have 0 copies

//     for (mapa_dim::iterator it_dim = objects.second.begin();
//          it_dim != objects.second.end(); ++it_dim)
//         if (it_dim->second == 0)
//             objects.second.erase(it_dim);  // eliminate dimers that have 0
//                                            // copies
...
//     for (mapa_owm::iterator it_owm = objects_with_mets.begin();
//          it_owm != objects_with_mets.end(); ++it_owm)
//         if (it_owm->second == 0)
...
////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
// METABOLISM to delete
// int ToyPlugin::metabolism(const std::vector<std::pair<int, int> >& genotype,
//                           std::pair<mapa_prot, mapa_dim> objects,
//                           mapa_met& met) {
//     int gene_number = genotype.size();
//     mapa_owm objects_with_mets;
//     int cuenta = 0;  // if it hasn't broken the Met in 2^n steps, then it won't
//                      // be able to do it
//     double bas = 0.0;
//     while (1) {  // we loop over the cycle until the metabolite is broken or
//                  // it's let go
...
//         if (it_met1 != met.end() &&
//             it_met1->first.size() < 8)  // that means something is broken
//             return 1;
//         met.clear();  // at the end of the cycle, all metabolites that haven't
//                       // been bound by anything disappear
//         // Now the P+M and D+M will become part of Met too (the Ps and Ds will
//         // disappear)
//         for (mapa_owm::iterator it = objects_with_mets.begin();
//              it != objects_with_mets.end(); ++it)
//             met[it->first.met] += it->second;
//         objects_with_mets
//             .clear();  // and all objects with mets will disappear of course
//         ++cuenta;
//         if (cuenta >
//             int_pow(2, gene_number))  // if it hasn't broken the Met in 2^n
//                                       // steps, then it won't be able to do it
//             return 0; 
//     }  // while
// }
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
// PHENOTYPE to delete
// std::vector<int> ToyPlugin::vec_phenotype(
//     const std::vector<std::pair<int, int> >& genotype,
//     const std::vector<std::string>& env) {
//     // ALGORITHM
//     // COMPUTE THE CYCLE FIRST (WITHOUT METABOLITES)
//     std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim> > > cycle =
//         boolean_network(genotype);
...
//             met[env[m]]++;  // this will allow us to generalize for more than
//                             // one metabolite
...
////////////////////////////////////////////////////////////////////////
// STR PHENOTYPE
std::vector<int> ToyPlugin::metabolic_phenotype(
    const std::string& genotype, const std::vector<std::string>& env) {
    std::vector<std::pair<int, int> > vec_genotype =
        genotype_str_to_vec(genotype);
...
double ToyPlugin::metabolic_robustness(const std::string& genotype,
                                       const std::vector<int>& phen,
                                       const std::vector<std::string>& env) {
    double rob = 0.0;
...
double ToyPlugin::regulatory_robustness(const std::string& genotype,
                                        const std::vector<int>& logic_vector) {
    double rob = 0.0;
    int gensize = genotype.size();
    for (int g = 0; g < gensize; ++g) {
        std::string mutgen = mutation(genotype, g);
...
std::map<std::vector<int>, int> ToyPlugin::metabolic_evolvability(
    const std::string& genotype, const std::vector<int>& phen,
    const std::vector<std::string>& env) {
...
std::map<std::vector<int>, int> ToyPlugin::regulatory_evolvability(
...
std::vector<std::pair<int, int> > ToyPlugin::genotype_str_to_vec(
    ...
    for (int g = 0; g < gene_number; ++g) {
        ...
        //std::cout << prot_gen[vec_genotype[g].second] << " ";
    }
    //std::cout<<"\n";
